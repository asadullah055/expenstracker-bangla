    ...
    return (
        <div className='flex'>
            <div className="w-screen h-screen md:w-[60vw] px-12 pt-8 pb-12">
                <h2 className='text-lg font-medium text-black'>খরচ ট্র্যাকার</h2>
                {children}
            </div>
            ...
            <div className="hidden md:block w-[40vw] h-screen bg-violet-50 bg-auth-bg-img bg-no-repeat bg-center overflow-hidden p-8 relative">
                ...
                <div className="grid grid-cols-1 z-20">
                    <StatsInfoCard
                        ...
                        label="আপনার আয় ও ব্যয় ট্র্যাক করুন"
                        value={430000}
                        color="bg-primary"
                    />
                </div>
...
const StatsInfoCard = ({ icon, label, value, color }) => {
    return (
        <div className="flex gap-6 bg-white p-4 rounded-xl shadow-md shadow-purple-400/10 border border-gray-200/50 z-10">
            ...
            <div className={`w-12 h-12 flex items-center justify-center text-[26px] ${color} rounded-full text-white drop-shadow-xl`}>
                {icon}
            </div>
            <div>
                <h6 className="text-xs text-gray-500 mb-1">{label}</h6>
                <span className="text-[20px]">৳{addThousandSeparator(value)}</span>
            </div>
